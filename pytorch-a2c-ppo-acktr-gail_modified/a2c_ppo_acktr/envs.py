...
import mlca.gridworld_environments as environments
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, set_time_limit=None):
    def _thunk():
        ...
        if str(env.__class__.__name__).find('TimeLimit') >= 0 or set_time_limit is not None:
...
class VecPyTorch(VecEnvWrapper):
    ...
    def step_async(self, actions):
        if isinstance(actions, torch.LongTensor) or (torch.cuda.is_available() and isinstance(actions, torch.cuda.LongTensor)):
